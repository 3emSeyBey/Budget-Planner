...
class BudgetManager {
  ...
  async getWeeklyBudget(weekDate) {
    const sql = `
      ...
      WHERE wb.week_date = ? 
      GROUP BY wb.id, wb.week_date, wb.category_id, wb.planned_amount, wb.notes, wb.created_at, wb.updated_at, bc.name, bc.bank, bc.is_essential, bc.priority_order
  ...
  async updateWeeklyBudgetLimit(weeklyBudgetLimit) {
    const sql = `
      INSERT INTO weekly_summaries (week_date, total_planned, weekly_budget_limit, created_at, updated_at)
      VALUES (CURDATE(), 0, ?, NOW(), NOW())
      ON DUPLICATE KEY UPDATE 
        weekly_budget_limit = VALUES(weekly_budget_limit),
        updated_at = NOW()
    `;
    
    await query(sql, [weeklyBudgetLimit]);
