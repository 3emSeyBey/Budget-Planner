...
class ExpenseTracker {
  ...
  /**
   * Get expenses by date range (Wednesday to Wednesday)
   */
  async getExpensesByDateRange(startDate, endDate) {
    const sql = `
      ...
      ORDER BY e.week_date DESC, e.created_at DESC
    `;
  ...
  async getWeeklyExpenseSummary(weekDate) {
    const sql = `
      SELECT 
        ...
        COALESCE(wb.planned_amount, 0) as planned_amount,
        (COALESCE(SUM(e.amount), 0) - COALESCE(wb.planned_amount, 0)) as variance
      ...
      GROUP BY bc.id, bc.name, bc.bank, bc.priority_order, wb.planned_amount
  ...
  async getSpendingTrends(weeks = 4) {
    const sql = `
      ...
      FROM expenses 
      WHERE week_date >= date('now', '-' || ? || ' weeks')
      GROUP BY week_date 
   ...
   * Update expense
   */
  async updateExpense(expenseId, weekDate, categoryId, amount, description = '', paymentMethod = '', location = '') {
    ...
    if (expense.length > 0) {
      const oldWeekDate = expense[0].week_date;
      const oldCategoryId = expense[0].category_id;
      
      // Update the expense
      const updateSql = `
        UPDATE expenses 
        SET week_date = ?, category_id = ?, amount = ?, description = ?, payment_method = ?, location = ?
        WHERE id = ?
      `;
      
      const result = await query(updateSql, [weekDate, categoryId, amount, description, paymentMethod, location, expenseId]);
      
      if (result.affectedRows > 0) {
        // Update budget actual amounts for both old and new week/category
        await this.updateBudgetActual(oldWeekDate, oldCategoryId);
        await this.updateBudgetActual(weekDate, categoryId);
        return true;
      }
    }
